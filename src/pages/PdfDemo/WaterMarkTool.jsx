...
const WaterMarkTool = () => {
  ...
  const handleMergeClick = async () => {
    ...
    if (data.length >= 1) {
      ...
      sendMergeRequest({ pdf_data: fileDataArray,water_mark_text:formData.water_mark_text });
    } else {
  ...
  const sendMergeRequest = (data_post) => {
    ...
    const subscription = post('pdf/watermark_pdf', data_post).subscribe((response) => {
    //  setLoading(false);
      showNotification("success", "Watermark Completed Successfully");
      setResult(true);
      ...
      <div class="card">
        <div className="card-upload">
          <header className="card-header">
            <div className="table-style">
              <p className="has-text-danger is-size-7">
                <u> Note: Upload First file template pdf which has header and footer.
                </u></p>
          ...
          <div className="content">
            <div className="field">
              <label className="label">Enter Water Mark Text</label>              
              <div className="control">
                <input
                  type="text"
                  id="inputField"
                  className="input"
                  value={formData.water_mark_text}
                  onChange={(e) => handleInputChange("water_mark_text", e.target.value)}
                />
                <p class="help is-info">From this page uploaded pdf content will be overlayed</p>
              </div>
            </div>
